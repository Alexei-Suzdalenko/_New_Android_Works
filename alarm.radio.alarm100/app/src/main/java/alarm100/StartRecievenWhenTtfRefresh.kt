...
class StartRecievenWhenTtfRefresh : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == Intent.ACTION_BOOT_COMPLETED) {
            val i = Intent(context, Servce_radio_budilnik::class.java)
            i.action = "com.test.andsrvfg.AndSrvFgService"
            ContextCompat.startForegroundService(context ,i)
        }
    }

    // si el telefono estaba no activo... ponemos la projima fecha mas alante en un dia () teoricamente
